...
#include "ns3/bitcoin-topology-snapshot.h"
...
BitcoinTopologyHelper::BitcoinTopologyHelper(uint32_t noCpus,
											 uint32_t totalNoNodes,
											 uint32_t noMiners,
											 enum BitcoinRegion *minersRegions,
											 enum Cryptocurrency cryptocurrency,
  ...
  std::array<double, 7>
	  nodesDistributionIntervals{NORTH_AMERICA, EUROPE, SOUTH_AMERICA, ASIA_PACIFIC, JAPAN, AUSTRALIA, OTHER};
  ...
  switch (m_cryptocurrency) {
	case BITCOIN: {
	  ...
	  m_nodesDistribution = std::piecewise_constant_distribution<double>(nodesDistributionIntervals.begin(),
  ...
  m_connectionsDistribution = std::piecewise_constant_distribution<double>(connectionsDistributionIntervals.begin(),
																		   connectionsDistributionIntervals.end(),
																		   connectionsDistributionWeights.begin());

  m_europeDownloadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(downloadBandwitdhIntervals.begin(),
												   downloadBandwitdhIntervals.end(),
												   EuropeDownloadWeights.begin());
  m_europeUploadBandwidthDistribution = std::piecewise_constant_distribution<double>(uploadBandwitdhIntervals.begin(),
																					 uploadBandwitdhIntervals.end(),
																					 EuropeUploadWeights.begin());
  m_northAmericaDownloadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(downloadBandwitdhIntervals.begin(),
												   downloadBandwitdhIntervals.end(),
												   NorthAmericaDownloadWeights.begin());
  m_northAmericaUploadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(uploadBandwitdhIntervals.begin(),
												   uploadBandwitdhIntervals.end(),
												   NorthAmericaUploadWeights.begin());
  m_asiaPacificDownloadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(downloadBandwitdhIntervals.begin(),
												   downloadBandwitdhIntervals.end(),
												   AsiaPacificDownloadWeights.begin());
  m_asiaPacificUploadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(uploadBandwitdhIntervals.begin(),
												   uploadBandwitdhIntervals.end(),
												   AsiaPacificUploadWeights.begin());
  m_japanDownloadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(downloadBandwitdhIntervals.begin(),
												   downloadBandwitdhIntervals.end(),
												   JapanDownloadWeights.begin());
  m_japanUploadBandwidthDistribution = std::piecewise_constant_distribution<double>(uploadBandwitdhIntervals.begin(),
																					uploadBandwitdhIntervals.end(),
																					JapanUploadWeights.begin());
  m_southAmericaDownloadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(downloadBandwitdhIntervals.begin(),
												   downloadBandwitdhIntervals.end(),
												   SouthAmericaDownloadWeights.begin());
  m_southAmericaUploadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(uploadBandwitdhIntervals.begin(),
												   uploadBandwitdhIntervals.end(),
												   SouthAmericaUploadWeights.begin());
  m_australiaDownloadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(downloadBandwitdhIntervals.begin(),
												   downloadBandwitdhIntervals.end(),
												   AustraliaDownloadWeights.begin());
  m_australiaUploadBandwidthDistribution =
	  std::piecewise_constant_distribution<double>(uploadBandwitdhIntervals.begin(),
												   uploadBandwitdhIntervals.end(),
												   AustraliaUploadWeights.begin());
...
/* 		if (m_systemId == 0)
	  ...
	  } else if (std::find(m_nodesConnections[i].begin(), m_nodesConnections[i].end(), candidatePeer)
  ...
  if (m_systemId == 0) {
	for (int i = 0; i < m_totalNoNodes; i++) {
	  if (m_nodesConnections[i].size() < m_minConnections[i])
		std::cout << "Node " << i << " should have at least " << m_minConnections[i] << " connections but it has only "
...
/* 	if (m_systemId == 0)
  ...
  if (m_systemId == 0) {
	...
	for (auto region : downloadRegionBandwidths) {
	  ...
	  std::cout << "The download speed for region " << getBitcoinRegion(getBitcoinEnum(region.first)) << " = "
				<< average / region.second.size() << " Mbps\n";
	}
	...
	for (auto region : uploadRegionBandwidths) {
	  ...
	  std::cout << "The upload speed for region " << getBitcoinRegion(getBitcoinEnum(region.first)) << " = "
				<< average / region.second.size() << " Mbps\n";
	}
  }
  ...
  for (auto miner = m_miners.begin(); miner != m_miners.end(); miner++) {
	...
	for (std::vector<uint32_t>::const_iterator it = m_nodesConnections[*miner].begin();
	  ...
	  if (*it > *miner)    //Do not recreate links
	  {
		...
		if (m_latencyParetoShapeDivider > 0) {
		  ...
		  paretoDistribution->SetAttribute("Mean",
										   DoubleValue(m_regionLatencies[m_bitcoinNodesRegion[(m_nodes.at(*miner).Get(0))->GetId()]]
													   [m_bitcoinNodesRegion[(m_nodes.at(*it).Get(0))->GetId()]]));
		  paretoDistribution->SetAttribute("Shape",
										   DoubleValue(
											   m_regionLatencies[m_bitcoinNodesRegion[(m_nodes.at(*miner).Get(0))->GetId()]]
											   [m_bitcoinNodesRegion[(m_nodes.at(*it).Get(0))->GetId()]]
	...
	for (std::vector<uint32_t>::const_iterator it = node.second.begin(); it != node.second.end(); it++) {
		  ...
		  std::find(m_miners.begin(), m_miners.end(), node.first) == m_miners.end()))    //Do not recreate links
	  {
		...
		if (m_latencyParetoShapeDivider > 0) {
		  Ptr<ParetoRandomVariable> paretoDistribution = CreateObject<ParetoRandomVariable>();
		  paretoDistribution->SetAttribute("Mean",
										   DoubleValue(m_regionLatencies[m_bitcoinNodesRegion[(m_nodes.at(node.first).Get(
											   0))->GetId()]]
													   [m_bitcoinNodesRegion[(m_nodes.at(*it).Get(0))->GetId()]]));
		  paretoDistribution->SetAttribute("Shape",
										   DoubleValue(
											   m_regionLatencies[m_bitcoinNodesRegion[(m_nodes.at(node.first).Get(0))->GetId()]]
											   [m_bitcoinNodesRegion[(m_nodes.at(*it).Get(0))->GetId()]]
												   / m_latencyParetoShapeDivider));
		  latencyStringStream << paretoDistribution->GetValue() << "ms";
		} else {
