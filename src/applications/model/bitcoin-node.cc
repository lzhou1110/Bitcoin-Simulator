...
// Application Methods
    ...
    BitcoinNode::StartApplication()    // Called at time specified by Start
    {
        ...
        NS_LOG_INFO("Node " << GetNode()->GetId() << ": m_invTimeoutMinutes = " << m_invTimeoutMinutes.GetMinutes()
                            << "mins");
    ...
    BitcoinNode::StopApplication()     // Called at time specified by Stop
    {
        ...
        NS_LOG_WARN("Mean Block Receive Time = " << m_meanBlockReceiveTime << " or "
                                                 << static_cast<int>(m_meanBlockReceiveTime) / m_secondsPerMin
                                                 << "min and "
                ...
                while ((pos = totalReceivedData.find(delimiter)) != std::string::npos) {
                    ...
                    switch (d["message"].GetInt()) {
                        case INV: {
                            ...
                            m_nodeStats->invReceivedBytes +=
                                    m_bitcoinMessageHeader + m_countBytes + d["inv"].Size() * m_inventorySizeBytes;
                   ...
                   */
                                    ...
                                    if (m_invTimeouts.find(parsedInv) == m_invTimeouts.end()) {
                                        ...
                                        timeout = Simulator::Schedule(m_invTimeoutMinutes,
                                                                      &BitcoinNode::InvTimeoutExpired, this, parsedInv);
                        ...
                        case EXT_INV: {
                            ...
                            m_nodeStats->extInvReceivedBytes +=
                                    m_bitcoinMessageHeader + m_countBytes + d["inv"].Size() * m_inventorySizeBytes;
                                    ...
                                    if (m_queueChunks[blockHash].size() > 0) {
                                        ...
                                        if (d["inv"][j]["fullBlock"].GetBool()) {
                                            ...
                                            for (int k = 0; k < d["inv"][j]["availableChunks"].Size(); k++) {
                                                ...
                                                if (std::find(m_queueChunks[blockHash].begin(),
                                                              m_queueChunks[blockHash].end(),
                                                              d["inv"][j]["availableChunks"][k].GetInt()) !=
                                                    m_queueChunks[blockHash].end())
                                                    candidateChunks.push_back(
                                                            d["inv"][j]["availableChunks"][k].GetInt());
                                            }
                                        }
...
/*                     std::cout << "candidateChunks = ";
                    ...
                    std::cout << "\n"; */
                                        ...
                                        if (candidateChunks.size() > 0) {
                                            ...
                                            NS_LOG_INFO("EXT_INV: Bitcoin node " << GetNode()->GetId()
                                                                                 << " will request the chunk with index = "
                                                                                 << randomIndex << " and value = "
                                                                                 << candidateChunks[randomIndex]);
                                            ...
                                            timeout = Simulator::Schedule(Minutes(m_invTimeoutMinutes.GetMinutes() /
                                                                                  ceil(blockSize /
                                                                                       static_cast<double>(m_chunkSize))),
                                                                          &BitcoinNode::ChunkTimeoutExpired, this,
                                                                          chunk.str());
...
/*                     PrintQueueChunks();
                    ...
                    PrintReceivedChunks(); */
                            ...
                            if (!requestChunks.empty()) {
                                ...
                                for (auto chunk_it = requestChunks.begin();
                                     chunk_it < requestChunks.end(); chunk_it++) {
                        ...
                        case EXT_GET_HEADERS: {
                            ...
                            for (j = 0; j < d["blocks"].Size(); j++) {
                                ...
                                if (m_blockchain.HasBlock(height, minerId) || m_blockchain.IsOrphan(height, minerId)) {
                                    ...
                                    NS_LOG_INFO("EXT_GET_HEADERS: Bitcoin node " << GetNode()->GetId()
                                                                                 << " has received only the headers of the block with hash = "
                                                                                 << blockHash);
                                    requestHeaders.push_back(m_onlyHeadersReceived[blockHash]);
                                } else {
                                    NS_LOG_INFO("EXT_GET_HEADERS: Bitcoin node " << GetNode()->GetId()
                                                                                 << " has neither the block nor the headers of the block hash = "
                                                                                 << blockHash);
                            ...
                            if (!requestHeaders.empty()) {
                                ...
                                for (block_it = requestHeaders.begin(); block_it < requestHeaders.end(); block_it++) {
                                    ...
                                    if (m_blockchain.HasBlock(block_it->GetBlockHeight(), block_it->GetMinerId())
                                        ...
                                        int noChunks = ceil(
                                                block_it->GetBlockSizeBytes() / static_cast<double>(m_chunkSize));
                        ...
                        case GET_DATA: {
                            ...
                            m_nodeStats->getDataReceivedBytes +=
                                    m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size() * m_inventorySizeBytes;
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 ()
		  		          ...
		  		          << Simulator::Now ().GetSeconds() << "s \n"; */
                                ...
                                if (m_sendBlockTimes.size() == 0 ||
                                    Simulator::Now().GetSeconds() > m_sendBlockTimes.back()) {
                                    eventTime = 0;
                                } else {
                        ...
                        case EXT_GET_DATA: {
                            ...
                            m_nodeStats->extGetDataReceivedBytes +=
                                    m_bitcoinMessageHeader + m_countBytes + d["chunks"].Size() * m_inventorySizeBytes;
                            ...
                            for (j = 0; j < d["chunks"].Size(); j++) {
                                ...
                                if (m_blockchain.HasBlock(height, minerId) || m_blockchain.IsOrphan(height, minerId) ||
                                    ...
                                    NS_LOG_INFO("EXT_GET_DATA: Bitcoin node " << GetNode()->GetId()
                                                                              << " has received the headers (and maybe some chunks) of the block with hash = "
                                                                              << blockHash);
                                    ...
                                    if (d["chunks"][j]["fullBlock"].GetBool()) {
                                        ...
                                        for (int k = 0; k < d["chunks"][j]["availableChunks"].Size(); k++) {
                                            if (std::find(m_queueChunks[blockHash].begin(),
                                                          m_queueChunks[blockHash].end(),
                                                          d["chunks"][j]["availableChunks"][k].GetInt()) !=
                                                m_queueChunks[blockHash].end())
                                                candidateChunks.push_back(
                                                        d["chunks"][j]["availableChunks"][k].GetInt());
                                        }
                    ...
                    std::cout << "\n"; */
                            ...
                            if (!requestedChunks.empty()) {
                                ...
                                for (auto &requestedChunk : requestedChunks) {
                                    ...
                                    if (m_blockchain.HasBlock(height, minerId) ||
                                        m_blockchain.IsOrphan(height, minerId)) {
                                    ...
                                    if (chunkId ==
                                        ceil(newBlock.GetBlockSizeBytes() / static_cast<double>(m_chunkSize) - 1) &&
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 ()
		  		          ...
		  		          << Simulator::Now ().GetSeconds() << "s \n"; */
                                ...
                                //std::cout << sendTime << " " << eventTime << " " << m_sendBlockTimes.size() << std::endl;
                                NS_LOG_INFO("Node " << GetNode()->GetId() << " will start sending the chunk to "
                                                    << InetSocketAddress::ConvertFrom(from).GetIpv4()
                                                    << " at " << Simulator::Now().GetSeconds() + eventTime << "\n");
                        ...
                        case HEADERS: {
                            ...
                            m_nodeStats->headersReceivedBytes +=
                                    m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size() * m_headersSizeBytes;
                            ...
                            for (j = 0; j < d["blocks"].Size(); j++) {
                                ...
                                Block newBlockHeaders(d["blocks"][j]["height"].GetInt(),
                                                      d["blocks"][j]["minerId"].GetInt(),
                                                      d["blocks"][j]["parentBlockMinerId"].GetInt(),
                                                      d["blocks"][j]["size"].GetInt(),
                                                      d["blocks"][j]["timeCreated"].GetDouble(),
                                                      Simulator::Now().GetSeconds(),
                                                      InetSocketAddress::ConvertFrom(from).GetIpv4());
                                m_onlyHeadersReceived[blockHash] = Block(d["blocks"][j]["height"].GetInt(),
                                                                         d["blocks"][j]["minerId"].GetInt(),
                                                                         d["blocks"][j]["parentBlockMinerId"].GetInt(),
                                                                         d["blocks"][j]["size"].GetInt(),
                                                                         d["blocks"][j]["timeCreated"].GetDouble(),
                                ...
                                if (m_protocolType == SENDHEADERS && !m_blockchain.HasBlock(height, minerId) &&
                                    !m_blockchain.IsOrphan(height, minerId) && !ReceivedButNotValidated(blockHash)) {
                                    NS_LOG_INFO("We have not received an INV for the block with height = "
                                                        << d["blocks"][j]["height"].GetInt()
                   ...
                   */
                                    ...
                                    if (m_invTimeouts.find(blockHash) == m_invTimeouts.end()) {
                                        ...
                                        timeout = Simulator::Schedule(m_invTimeoutMinutes,
                                                                      &BitcoinNode::InvTimeoutExpired, this, blockHash);
                                        m_invTimeouts[blockHash] = timeout;
                                    } else {
                                        NS_LOG_INFO("HEADERS: Bitcoin node " << GetNode()->GetId()
                                                                             << " has already requested the block");
                                ...
                                if (!m_blockchain.HasBlock(parentHeight, parentMinerId) &&
                                    !m_blockchain.IsOrphan(parentHeight, parentMinerId) &&
                   ...
                   */
                                    ...
                                    if (m_invTimeouts.find(parentBlockHash) == m_invTimeouts.end()) {
                                        ...
                                        if (m_protocolType == STANDARD_PROTOCOL ||
                                            (m_protocolType == SENDHEADERS &&
                                             std::find(requestBlocks.begin(), requestBlocks.end(), parentBlockHash) ==
                                             requestBlocks.end())) {
                                            if (!OnlyHeadersReceived(parentBlockHash))
                                                requestHeaders.push_back(parentBlockHash.c_str());
                                            timeout = Simulator::Schedule(m_invTimeoutMinutes,
                                                                          &BitcoinNode::InvTimeoutExpired, this,
                                                                          parentBlockHash);
	               ...
	               */
                        ...
                        case EXT_HEADERS: {
                            ...
                            m_nodeStats->extHeadersReceivedBytes +=
                                    m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size() * m_headersSizeBytes;
                                    ...
                                    if (m_queueChunks[blockHash].size() > 0) {
                                        ...
                                        if (d["blocks"][j]["fullBlock"].GetBool()) {
                                            ...
                                            for (int k = 0; k < d["blocks"][j]["availableChunks"].Size(); k++) {
                                                if (std::find(m_queueChunks[blockHash].begin(),
                                                              m_queueChunks[blockHash].end(),
                                                              d["blocks"][j]["availableChunks"][k].GetInt()) !=
                                                    m_queueChunks[blockHash].end())
                                                    candidateChunks.push_back(
                                                            d["blocks"][j]["availableChunks"][k].GetInt());
                                            }
...
/*                     std::cout << "candidateChunks = ";
                    ...
                    std::cout << "\n"; */
                                        ...
                                        if (candidateChunks.size() > 0 &&
                                            std::find(m_queueChunkPeers[blockHash].begin(),
                                                      m_queueChunkPeers[blockHash].end(), from) ==
                                            m_queueChunkPeers[blockHash].end()) {
                                            int randomIndex = rand() % candidateChunks.size();
                                            NS_LOG_INFO("EXT_HEADERS: Bitcoin node " << GetNode()->GetId()
                                                                                     << " will request the chunk with index = "
                                                                                     << randomIndex << " and value = "
                                            ...
                                            if (std::find(m_queueChunkPeers[blockHash].begin(),
                                                          m_queueChunkPeers[blockHash].end(), from) ==
                                                m_queueChunkPeers[blockHash].end())
                                                NS_LOG_INFO("EXT_HEADERS: Bitcoin node " << GetNode()->GetId()
                                                                                         << " will not request any chunks from this peer, because it has already all the available ones");
...
/*                     PrintQueueChunks();
                   ...
                   */
                                    ...
                                    if (m_queueChunks.find(parentBlockHash) == m_queueChunks.end() ||
                                        std::find(m_queueChunkPeers[parentBlockHash].begin(),
                                                  m_queueChunkPeers[parentBlockHash].end(), from) ==
                                        m_queueChunkPeers[parentBlockHash].end()) {
                                        NS_LOG_INFO("EXT_HEADERS: Bitcoin node " << GetNode()->GetId()
                                    ...
                                    if (m_protocolType == STANDARD_PROTOCOL ||
                                        (m_protocolType == SENDHEADERS &&
                                         std::find(requestChunks.begin(), requestChunks.end(), parentBlockHash) ==
                                         requestChunks.end()))
                                        m_queueInv[parentBlockHash].push_back(from);
	               ...
	               */
                        ...
                        case BLOCK: {
                            ...
                            double eventTime = 0;
                            double minSpeed = std::min(m_downloadSpeed,
                                                       m_peersUploadSpeeds[InetSocketAddress::ConvertFrom(
                                                               from).GetIpv4()] * 1000000 / 8);
                            ...
                            for (int j = 0; j < d["blocks"].Size(); j++) {
                                if (blockType == "block")
                                    ...
                                    int noTransactions = static_cast<int>(
                                            (d["blocks"][j]["size"].GetInt() - m_blockHeadersSizeBytes) /
                                            m_averageTransactionSize);
                            ...
                            NS_LOG_INFO(m_downloadSpeed << " " << m_peersUploadSpeeds[InetSocketAddress::ConvertFrom(
                                    from).GetIpv4()] * 1000000 / 8 << " " << minSpeed);

                            std::string help = blockInfo.GetString();

                            if (blockType == "block") {
                                if (m_receiveBlockTimes.size() == 0 ||
                                    Simulator::Now().GetSeconds() > m_receiveBlockTimes.back()) {
                                    receiveTime = blockMessageSize / m_downloadSpeed;
                                    eventTime = blockMessageSize / minSpeed;
                                } else {
                                    receiveTime = blockMessageSize / m_downloadSpeed + m_receiveBlockTimes.back() -
                                                  Simulator::Now().GetSeconds();
                                    eventTime = blockMessageSize / minSpeed + m_receiveBlockTimes.back() -
                                                Simulator::Now().GetSeconds();
                                }
                                m_receiveBlockTimes.push_back(Simulator::Now().GetSeconds() + receiveTime);


                                Simulator::Schedule(Seconds(eventTime), &BitcoinNode::ReceivedBlockMessage, this, help,
                                                    from);
                                Simulator::Schedule(Seconds(receiveTime), &BitcoinNode::RemoveReceiveTime, this);
                            } else if (blockType == "compressed-block") {
                                if (m_receiveCompressedBlockTimes.size() == 0 ||
                                    Simulator::Now().GetSeconds() > m_receiveCompressedBlockTimes.back()) {
                                    receiveTime = blockMessageSize / m_downloadSpeed;
                                    eventTime = blockMessageSize / minSpeed;
                                } else {
                                    receiveTime =
                                            blockMessageSize / m_downloadSpeed + m_receiveCompressedBlockTimes.back() -
                                            Simulator::Now().GetSeconds();
                                    eventTime = blockMessageSize / minSpeed + m_receiveCompressedBlockTimes.back() -
                                                Simulator::Now().GetSeconds();
                                }
                                ...
                                Simulator::Schedule(Seconds(eventTime), &BitcoinNode::ReceivedBlockMessage, this, help,
                                                    from);
                                Simulator::Schedule(Seconds(receiveTime),
                                                    &BitcoinNode::RemoveCompressedBlockReceiveTime, this);
                            }

                            NS_LOG_INFO(
                                    "BLOCK:  Node " << GetNode()->GetId() << " will receive the full block message at "
                        ...
                        case CHUNK: {
                            ...
                            for (int j = 0; j < d["chunks"].Size(); j++) {
                                ...
                                m_nodeStats->chunkReceivedBytes +=
                                        chunkMessageSize + 1 + 1;//the requested chunk + the fullBlock
                                if (!d["chunks"][j]["fullBlock"].GetBool())
                            ...
                            NS_LOG_INFO("CHUNK: At time " << Simulator::Now().GetSeconds()
                                                          << " Node " << GetNode()->GetId()
                                                          << " received a chunk message " << chunkInfo.GetString());
                            ...
                            if (m_receiveBlockTimes.size() == 0 ||
                                ...
                                receiveTime = chunkMessageSize / m_downloadSpeed + m_receiveBlockTimes.back() -
                                              Simulator::Now().GetSeconds();
                                eventTime = chunkMessageSize / minSpeed + m_receiveBlockTimes.back() -
                                            Simulator::Now().GetSeconds();
                            }
                            m_receiveBlockTimes.push_back(Simulator::Now().GetSeconds() + receiveTime);

                            NS_LOG_INFO(
                                    "CHUNK:  Node " << GetNode()->GetId() << " will receive the full chunk message at "
                                                    << Simulator::Now().GetSeconds() + eventTime);
                            Simulator::Schedule(Seconds(eventTime), &BitcoinNode::ReceivedChunkMessage, this, help,
                                                from);
                            Simulator::Schedule(Seconds(receiveTime), &BitcoinNode::RemoveReceiveTime, this);
    ...
    BitcoinNode::ReceivedBlockMessage(std::string &blockInfo, Address &from) {
        ...
        NS_LOG_INFO("ReceivedBlockMessage: At time " << Simulator::Now().GetSeconds()
                                                     << " Node " << GetNode()->GetId() << " received a block message "
                                                     << blockInfo);
        ...
        for (int j = 0; j < d["blocks"].Size(); j++) {
            ...
            if (!m_blockchain.HasBlock(parentHeight, parentMinerId) &&
                ...
                Block newBlock(d["blocks"][j]["height"].GetInt(), d["blocks"][j]["minerId"].GetInt(),
                               d["blocks"][j]["parentBlockMinerId"].GetInt(),
...
/*     PrintQueueChunks();
    ...
    PrintOnlyHeadersReceived(); */
            ...
            if (!m_blockchain.HasBlock(height, minerId) && !m_blockchain.IsOrphan(height, minerId) &&
                ...
                if (std::find(m_receivedChunks[blockHash].begin(), m_receivedChunks[blockHash].end(), chunkId) ==
                    m_receivedChunks[blockHash].end()) {
                    m_receivedChunks[blockHash].push_back(chunkId);

                    if (m_receivedChunks[blockHash].size() == 1 && m_spv)
                        AdvertiseFirstChunk(Block(d["chunks"][j]["height"].GetInt(), d["chunks"][j]["minerId"].GetInt(),
                                                  d["chunks"][j]["parentBlockMinerId"].GetInt(),
                                                  d["chunks"][j]["size"].GetInt(),
                                                  d["chunks"][j]["timeCreated"].GetDouble(),
                                                  Simulator::Now().GetSeconds(),
                                                  InetSocketAddress::ConvertFrom(from).GetIpv4()));

                    if (m_receivedChunks[blockHash].size() ==
                        ceil(d["chunks"][j]["size"].GetInt() / static_cast<double>(m_chunkSize))) {
                        if (!m_blockchain.HasBlock(parentHeight, parentMinerId) &&
                            !m_blockchain.IsOrphan(parentHeight, parentMinerId)
                            && !ReceivedButNotValidated(parentBlockHash) && !OnlyHeadersReceived(parentBlockHash)) {
                            NS_LOG_INFO("The Block with height = " << d["chunks"][j]["height"].GetInt()
                                                                   << " and minerId = "
                                                                   << d["chunks"][j]["minerId"].GetInt()
                                                                   << " is an orphan, so it will be discarded\n");
                        } else {
                            NS_LOG_INFO("The Block with height = " << d["chunks"][j]["height"].GetInt()
                                                                   << " and minerId = "
                            ...
                            Block newBlock(d["chunks"][j]["height"].GetInt(), d["chunks"][j]["minerId"].GetInt(),
                                           d["chunks"][j]["parentBlockMinerId"].GetInt(),
                        ...
                        for (int ii = 0; ii < d["chunks"][j]["requestChunks"].Size(); ii++) {
                            BitcoinChunk newChunk(d["chunks"][j]["height"].GetInt(), d["chunks"][j]["minerId"].GetInt(),
                                                  -1,
                                                  d["chunks"][j]["parentBlockMinerId"].GetInt(), //-1 if we are not going to request a chunk
                                                  d["chunks"][j]["size"].GetInt(),
                                                  d["chunks"][j]["timeCreated"].GetDouble(),
                                                  Simulator::Now().GetSeconds(),
                                                  InetSocketAddress::ConvertFrom(from).GetIpv4());
                        ...
                        if (d["chunks"][j]["fullBlock"].GetBool()) {
                            ...
                            for (int k = 0; k < d["chunks"][j]["availableChunks"].Size(); k++) {
                                if (std::find(m_queueChunks[blockHash].begin(), m_queueChunks[blockHash].end(),
                                              d["chunks"][j]["availableChunks"][k].GetInt()) !=
                                    m_queueChunks[blockHash].end())
          ...
          std::cout << "\n"; */
                        ...
                        if (candidateChunks.size() > 0) {
                            ...
                            if (d["chunks"][j]["requestChunks"].Size() == 0)
                                ...
                                for (int ii = 0; ii < d["chunks"][j]["requestChunks"].Size(); ii++) {
                                    BitcoinChunk newChunk(d["chunks"][j]["height"].GetInt(),
                                                          d["chunks"][j]["minerId"].GetInt(),
                                                          candidateChunks[randomIndex],
                                                          d["chunks"][j]["parentBlockMinerId"].GetInt(),
                                                          d["chunks"][j]["size"].GetInt(),
    ...
    PrintOnlyHeadersReceived(); */
        ...
        if (!chunkMessages.empty()) {
            ...
            for (auto &chunk : chunkMessages) {
                ...
                stringStream.clear();
                stringStream << chunk.first.GetBlockHeight() << "/" << chunk.first.GetMinerId() << "/"
                             << chunk.first.GetChunkId();
                chunkHash = stringStream.str();

                for (auto requestedChunk_it = chunk.second.begin();
                     requestedChunk_it != chunk.second.end(); requestedChunk_it++) {
                    rapidjson::Value requestChunks(rapidjson::kArrayType);
                    rapidjson::Value availableChunks(rapidjson::kArrayType);
                    ...
                    if (requestedChunk_it == chunk.second.begin() &&
                        chunk.first.GetChunkId() != -1) //Request it only once
                    {
            ...
            for (int j = 0; j < d["chunks"].Size(); j++) {
                if (d["chunks"][j]["chunk"].GetInt() ==
                    ceil(d["chunks"][j]["size"].GetInt() / static_cast<double>(m_chunkSize) - 1) &&
                    d["chunks"][j]["size"].GetInt() % m_chunkSize > 0)
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 ()
    ...
    BitcoinNode::ReceiveBlock(const Block &newBlock) {
        ...
        if (m_blockchain.HasBlock(newBlock) || m_blockchain.IsOrphan(newBlock) || ReceivedButNotValidated(blockHash)) {
            NS_LOG_INFO("ReceiveBlock: Bitcoin node " << GetNode()->GetId()
            ...
            NS_LOG_INFO("ReceiveBlock: Bitcoin node " << GetNode()->GetId()
                                                      << " has NOT added this block in the m_blockchain: " << newBlock);
    ...
    BitcoinNode::ReceivedLastChunk(const Block &newBlock) {
        ...
        if (m_blockchain.HasBlock(newBlock) || m_blockchain.IsOrphan(newBlock) || ReceivedButNotValidated(blockHash)) {
            NS_LOG_INFO("ReceivedLastChunk: Bitcoin node " << GetNode()->GetId()
                                                           << " has already added this block in the m_blockchain: "
                                                           << newBlock);
        } else {
            NS_LOG_INFO("ReceivedLastChunk: Bitcoin node " << GetNode()->GetId()
    ...
    BitcoinNode::ReceivedHigherBlock(const Block &newBlock) {
        NS_LOG_FUNCTION(this);
        NS_LOG_INFO("ReceivedHigherBlock: Bitcoin node " << GetNode()->GetId()
   ...
   */
    ...
    BitcoinNode::AdvertiseNewBlock(const Block &newBlock) {
        ...
        for (std::vector<Ipv4Address>::const_iterator i = m_peersAddresses.begin(); i != m_peersAddresses.end(); ++i) {
            if (*i != newBlock.GetReceivedFromIpv4()) {
                ...
                m_peersSockets[*i]->Send(reinterpret_cast<const uint8_t *>(packetInfo.GetString()),
                                         packetInfo.GetSize(), 0);
                m_peersSockets[*i]->Send(delimiter, 1, 0);

                if (m_protocolType == STANDARD_PROTOCOL)
                    m_nodeStats->invSentBytes +=
                            m_bitcoinMessageHeader + m_countBytes + d["inv"].Size() * m_inventorySizeBytes;
                else if (m_protocolType == SENDHEADERS)
                    m_nodeStats->headersSentBytes +=
                            m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size() * m_headersSizeBytes;

                NS_LOG_INFO("AdvertiseNewBlock: At time " << Simulator::Now().GetSeconds()
                                                          << "s bitcoin node " << GetNode()->GetId()
                                                          << " advertised a new Block: "
                                                          << newBlock << " to " << *i);
            }
        }
    }
    ...
    BitcoinNode::AdvertiseFullBlock(const Block &newBlock) {
        ...
        for (std::vector<Ipv4Address>::const_iterator i = m_peersAddresses.begin(); i != m_peersAddresses.end(); ++i) {
            ...
            m_peersSockets[*i]->Send(reinterpret_cast<const uint8_t *>(packetInfo.GetString()), packetInfo.GetSize(),
                                     0);
            m_peersSockets[*i]->Send(delimiter, 1, 0);

            if (m_protocolType == STANDARD_PROTOCOL) {
                m_nodeStats->extInvSentBytes +=
                        m_bitcoinMessageHeader + m_countBytes + d["inv"].Size() * m_inventorySizeBytes;
                ...
                m_nodeStats->extHeadersSentBytes +=
                        m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size() * m_headersSizeBytes;
                for (int j = 0; j < d["blocks"].Size(); j++) {
    ...
    void
    BitcoinNode::SendMessage(enum Messages receivedMessage, enum Messages responseMessage, rapidjson::Document &d,
        ...
        switch (d["message"].GetInt()) {
            ...
            case GET_DATA: {
                m_nodeStats->getDataSentBytes +=
                        m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size() * m_inventorySizeBytes;
                break;
            }
            case EXT_GET_DATA: {
                m_nodeStats->extGetDataSentBytes +=
                        m_bitcoinMessageHeader + m_countBytes + d["chunks"].Size() * m_inventorySizeBytes;
                for (int j = 0; j < d["chunks"].Size(); j++) {
    ...
    BitcoinNode::SendMessage(enum Messages receivedMessage, enum Messages responseMessage, rapidjson::Document &d,
        ...
        std::map < Ipv4Address, Ptr < Socket >> ::iterator
        it = m_peersSockets.find(outgoingIpv4Address);
        ...
        m_peersSockets[outgoingIpv4Address]->Send(reinterpret_cast<const uint8_t *>(buffer.GetString()),
                                                  buffer.GetSize(), 0);
    ...
    void
    BitcoinNode::SendMessage(enum Messages receivedMessage, enum Messages responseMessage, std::string packet,
                             Address &outgoingAddress) {
        NS_LOG_FUNCTION(this);
...
/*   PrintChunkTimeouts();
  ...
  PrintQueueChunkPeers(); */
    ...
    BitcoinNode::HasChunk(std::string blockHash, int chunk) {
        ...
        if (std::find(m_receivedChunks[blockHash].begin(), m_receivedChunks[blockHash].end(), chunk) !=
            m_receivedChunks[blockHash].end())
            return true;
        else
            return false;
    ...
    BitcoinNode::RemoveSendTime() {
        ...
        NS_LOG_INFO("RemoveSendTime: At Time " << Simulator::Now().GetSeconds() << " " << m_sendBlockTimes.front()
                                               << " was removed");
        m_sendBlockTimes.erase(m_sendBlockTimes.begin());
    }
    ...
    BitcoinNode::RemoveReceiveTime() {
        ...
        NS_LOG_INFO("RemoveReceiveTime: At Time " << Simulator::Now().GetSeconds() << " " << m_receiveBlockTimes.front()
                                                  << " was removed");
        m_receiveBlockTimes.erase(m_receiveBlockTimes.begin());
    }
    ...
    BitcoinNode::RemoveCompressedBlockReceiveTime() {
        ...
        NS_LOG_INFO("RemoveCompressedBlockReceiveTime: At Time " << Simulator::Now().GetSeconds() << " "
                                                                 << m_receiveCompressedBlockTimes.front()
                                                                 << " was removed");
